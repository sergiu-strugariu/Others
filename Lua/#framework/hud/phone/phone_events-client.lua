    ...
    if isPhoneActive or isForced ~= nil then
        ...
        if id ~= nil then
            ...
            if app.type == 'homepage' then
              return showHomepage(phoneScaleform, app.buttons, selectID, themeID)
            elseif app.type == 'contacts' then
                return openContactsMenu(phoneScaleform, app.buttons, appSelectID, app.name)
            elseif app.type == 'callscreen' then
                return openCallscreen(phoneScaleform, app.data.contact, app.data.pic, app.data.status, app.data.canAnswer)
            elseif app.type == 'emailList' then
                return openEmailsMenu(phoneScaleform, app.buttons, appSelectID, app.name)
            elseif app.type == 'emailView' then
                return openEmailView(phoneScaleform, tostring(app.data.title), tostring(app.data.from), tostring(app.data.to), tostring(app.data.message), app.data.canOpenMenu)
            elseif app.type == 'messagesList' then
                return openMessagesMenu(phoneScaleform, app.buttons, appSelectID, app.name)
            elseif app.type == 'messageView' then
                return openMessageView(phoneScaleform, tostring(app.data.contact), tostring(app.data.message), app.data.fromme, app.data.hasPic, app.data.canOpenMenu)
            elseif app.type == 'menu' then
                return openCustomMenu(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'settings' then
                return openSettingsList(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'todoList' then
                return openStatsMenu(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'todoView' then
                return openTodoView(phoneScaleform, app.name, app.data.title, app.data.line1, app.data.line2, app.data.footer)
            elseif app.type == 'numpad' then
                return openNumpadMenu(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'snapmatic' then
                return openSnapmatic(phoneScaleform)
            elseif app.type == 'missionStatsView' then
                return openMissionStatsView(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'gps' then
                return openGPSView(phoneScaleform, app.name)
            elseif app.type == 'trackifyView' then
                return openTrackifyView(phoneScaleform, app.name, app.buttons, appSelectID)
            elseif app.type == 'securoHack' then
                return openSecuroHackView(phoneScaleform, app.name, app.buttons, appSelectID)
...
AddEventHandler('scalePhone.GoBackApp', function(data)
    if appOpen ~= 0 then
        if data ~= nil and data.backApp ~= nil then
            ...
            if lastAppOpen == 2 then
                TriggerEvent('scalePhone.OpenApp', 0, false)
            end 
